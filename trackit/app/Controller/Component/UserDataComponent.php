...
class UserDataComponent extends Component {
	...
	*/
	...
	public function test_write($val)
	{
		$this->Session->write('test.write', $val);
	}
	...
	public function cleanLoginData($user = false) {
	...
	public function set_admin_mode($is_admin)
	{
		$this->Session->write('user.data.is_admin', $is_admin);
		if (!$is_admin)
		{
			$this->Session->delete('user.data.is_admin');
		}
	...
	public function isAdmin() {
		...
		$is_admin = $this->Session->read('user.data.is_admin');
		return $is_admin;
	}
	
	public function isAdminInDB() {
		...
		if ($user && $user['user_can_be_admin'])
		{
			return true;
		}
		else
	...
	public function updateFieldsInLoginData($params){
		if (empty($params)) {return false;}
		$uid = $this->getUserId();
		if(empty($uid)){return false;}
		
		foreach($params as $k=>$v){
			$this->Session->write("user.data.login.{$k}", $v);
		}
		return true;
	}
	
	public function getUserField($field_name)
	{
		$re=array('s'=>0,'val'=>'');
		$user = $this->getLoginData();
		
		if (!empty($field_name) && is_array($user) && array_key_exists($field_name, $user))
		{
			$re['s'] = 1;
			$re['val'] = $user[$field_name];
		}
		return $re;
	}
