 ...
 */
	...
	public function safe_update(){
		...
		$this->set('nopopup', false);
		
		$email = $this->UserData->getUserEmail();
		if (empty($email)){
			$msg = "Please <span style='text-decoration:underline;cursor:pointer' onclick='show_ulf();'>login</span> to update password. If you have forgotten your password, <span style='text-decoration:underline;cursor:pointer;' onclick=\"show_user_login_form(1);hide_and_show('.login_form', '.reset_pass_form');\">click here.</span>";
			$result['msg'] = $msg;
			$this->set('result', $result);
			$this->Session->setFlash($msg);
			
			$this->set('show_form', false);
			$this->set('nopopup', true);
			return;
		}
