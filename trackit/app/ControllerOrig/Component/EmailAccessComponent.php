...
class EmailAccessComponent extends Component {
	...
	*/
	...
	function shoot_email($reason = null, $data, $method = 'postmark') {
		...
		if($reason == 'confirmation_email_to_user_from_contact_us_page') {
			
			$data['subject'] = 'Thank you for contacting us';
			$data['from'] = EmailAccessComponent::TEAM_EMAIL;
			$data['fromName'] = 'Team '.GENERIC_APPNAME;
			$data['replyTo'] = EmailAccessComponent::TEAM_EMAIL;
			$data['replyToName'] = 'Team '.GENERIC_APPNAME;
		}
			...
			$data['subject'] = 'Confirm Email for '.GENERIC_APPNAME;
			$data['from'] = EmailAccessComponent::TEAM_EMAIL;
			$data['fromName'] = 'Team '.GENERIC_APPNAME;
			$data['replyTo'] = EmailAccessComponent::TEAM_EMAIL;
			$data['replyToName'] = 'Team '.GENERIC_APPNAME;
	...
	function format_email_options_and_send_email($method = null, $data_for_email = null) {
			...
			if(!empty($data_for_email['replyToName'])) {
				...
				$replyToName = 'Team '.GENERIC_APPNAME;
			}
			...
			$this->email->from(array($replyTo => $replyToName));
			$this->email->to($to);
			$this->email->cc($cc);
			$this->email->bcc($bcc);
