	...
	public function initialize($settings = array()) {
		...
		if ($this->status() == false) {
			...
			if ($this->userStatus() == false) {
				...
				if (!empty($settings['UserTokens']['oauth_token']) && !empty($settings['UserTokens']['oauth_token_secret']))
				{
					$this->_oauthToken = $settings['UserTokens']['oauth_token'];
					$this->_oauthTokenSecret = $settings['UserTokens']['oauth_token_secret'];
				}
				else if (!is_null($oauthSession)) {
 ...
 */
	...
	public function UploadApiRequest($method, $twitterMethodUrl, $params) {
		$request = array();
 ...
 */
	...
	public function updateStatusWithMedia($status, $options = array()) {
		if ($status != null || $status != '') {
			$body = Set::merge(array(
				'status' => $status
				), $options);

			//return json_decode($this->UploadApiRequest('post', $this->endPoint('statuses/update_with_media'), $body), true);
			return json_decode($this->apiRequest('post', $this->endPoint('statuses/update_with_media'), $body), true);
		}
	}
