...
if (empty($contents))
{
	$start_simple_coupon_link = SITE_NAME . "contents/add_static_coupon";
	
	echo "<div style='clear: both; background:#f3f2f2; color:black; padding: 5px; font-style:normal; font-weight:normal; font-size:16px;'>";
	echo "You haven't started any 'Simple Coupon Ad' yet.";
	echo "<br/>";
	//echo "<a href='/AB/open_campaigns/add' style=''> Lets start by adding an Adv Campaign </a>";
	echo "<a onmouseover=\"$(this).css('text-decoration', '1px dotted #ffffff');\" style='cursor:pointer; color:black; font-style:normal; font-weight:normal;text-decoration:underline;' href='{$start_simple_coupon_link}'> Create your first 'Simple Coupon Ad'.</a>";
	echo "</div>";
}
else
{
	echo $this->element('content/company_content_grid', array('contents' => $contents, 'aLayout' => 'static'));
}
/*				
