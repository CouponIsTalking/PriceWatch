...
echo "<div style=\"overflow:hidden;\">";
	...
	foreach ($contents as $index => $content)
	{
		...
		if ($type == 'news')
		{
			echo _get_content_picker_button_code($content_id);
			/*
		...
		else if ($type == 'image')
		{
			...
			*/
			...
			echo"	<img style='' src='{$single_encoded_fullpath}' onclick=\"update_image_in_ad('{$double_encoded_fullpath}', 0);\"></img>
					{$reduced_content}
				</content_image>
				";
		}
		else if ($type == 'video')
		{
			...
			if (!empty($fbobjectid))
			{
				...
				{
					$play_button_url = SITE_NAME . "/img/video_play_button.png";
					echo "<img onclick=\"zoomFBVideoFromCommentedIframe($(this).parent().find('div.__fb_vid_iframe').html());\" src='{$play_button_url}' style=\"cursor:pointer; background:URL('{$video_thumbnail_src}') center no-repeat;\"/>";
...
if (0==$num){
echo "<div class='__empty_items'></div>";
}
