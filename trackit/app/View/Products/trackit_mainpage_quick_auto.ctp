...
function trackit1234567890_loggedinuser()
{
	console.log("trackit1234567890_loggedinuser enter");
	$iframe = $("#trackit1234567890iframe2");
	console.log($iframe);
	
	$div = $iframe.find("input#user_logged_in");
	$logged_in_user = $div.val();
	console.log($div.val());
	console.log($div.text());
	console.log($logged_in_user);
	console.log()
	if (!($logged_in_user == 1 || $logged_in_user == "1"))
	{
		return;
	}
	...
	$pageurl = document.referrer; //$form2.find('#trackit1234567890pageurihidden').val();
	console.log($pageurl);
	...
	$TRACKER_SITE_NAME = "<?php echo SITE_NAME; ?>"; //"http://localhost/trackit/";
	$.ajax({
		type:"POST",
		data:{url: $pageurl, name: $name, email: $email, wait_price: $price, decode_title: 1},
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (result.success)
			{
				...
				$a.find("#msg").text("Congrats. We have saved this item to our tracker. You can browse and retrieve your saved items by going to savethisitem.com");
...
var iframeLoadExecuteInterval = setInterval(function(){iframeReadyEvent()},1000);

function iframeReadyEvent()
{
	if (typeof $ != 'undefined') {
		trackit1234567890_loggedinuser();
		clearInterval(iframeLoadExecuteInterval);
	}
	else
	{
		//continue;
...
//$(document).ready(trackit1234567890_loggedinuser);
...
if (!empty($user_email)){$logged_in_user = 1;} else {$logged_in_user = 0;}
echo "<input id='user_logged_in' style='display:none;' val='{$logged_in_user}'></input>";
?>
...
{
	echo "<form id='form2' style='display:none;'>";
}
else if (1 == $logged_in_user)
{
	echo "<form id='form2' style='display:block;'>";
}
?>
<div style='clear:both'></div>
...
<input id='trackit1234567890email' maxlength='400' name='trackit1234567890email' class='trackit1234567890email' style='width:300; float:left;' <?php if (!empty($user_email)){echo "disabled='true' val='{$user_email}'";}?>></input>
<br/>
<br/>
...
if (!empty($already_added) && $already_added == 1)
{
	echo "Congrats. We have saved this item to our tracker. You can browse and retrieve your saved items by going to savethisitem.com";
}
else
{
	echo "Click to add item.";
}
?>
