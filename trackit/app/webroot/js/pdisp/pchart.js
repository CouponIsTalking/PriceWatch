function clear_price_chart(){
	$('.graph_container').animate({opacity: 0},1000,function(){$("#chart").empty();$('.graph_container').css('opacity','1');});
}

function draw_price_chart($prodid){
$.ajax({type:'POST', data:{prodid:$prodid},url:$S_N+"product_prices/get_pdata",
success:function(data){render_price_chart(data);},
error:function(data){}
});
}

function render_price_chart($dp_history){
	
	$("#chart").empty();$('.graph_container');	
	if (!$dp_history || $dp_history.trim()==""){return;}
