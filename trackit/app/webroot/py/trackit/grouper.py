...
class Grouper:
	...
	def Algo1(self, group_map):
		...
		for props in filtered_props_list:
			...
			if look_for_color_root and self.WordsMeetsSizeExitCriteria(words_before_me): #(self.tag_dict.isColorHintWord(last_word) or self.MeetsColorExitCriteria(peid)):
				color_root = peid
	...
	def readWebElementAttr(eid, attr):
		
		eidmap = self.eidmap
		if attr in eidmap[eid]:
			val = eidmap[eid][attr]
		else:
			val = readWebElementAttr(self.alle[peid], attr)
			eidmap[eid][attr] = val
...
#				break
	...
	def FindTitle(self):
		...
		for e in title_elements:
			...
			if document_title:
				...
				if document_title.count(' ') == 0:
					if self.tag_dict.isApparelType(document_title):
						document_title = None			
		...
		for eid in eid_list_h_sorted:
			...
			clean_text = re.sub(' +',' ', text.encode("ascii", "ignore").rstrip().lstrip())
			if clean_text.count(' ') == 0:
				continue
