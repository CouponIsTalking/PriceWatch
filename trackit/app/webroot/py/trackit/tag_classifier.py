...
class Tags:
	...
	def isColorSquare(self, w, h):
		...
		# find elements of size less than 25x25
		if w > 30 or h > 30:
			return False
	...
	def isSizeType(self, size):
		...
		sizetypes2 = ['extra small', 'small', 'medium', 'large', 'extra large']
		
		# 1/2 letter size should be caps
		if size in sizetypes1:
			return True
		
		size = size.rstrip().lstrip().lower()
		size = re.sub(' +','', size)
		...
		if len(size) == 2:
			if size[0] >= '0' and size[0] <= '9' and size[1] >= '0' and size[1] <= '9':
				return True
		
		if len(size) == 3:
			if size[0] >= '0' and size[0] <= '9' and size[1] >= '0' and size[1] <= '9' and (size[2] == 'w' or size[2] == 'h'):
				return True
